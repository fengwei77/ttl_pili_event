  ...
  $.fn.flipster = function (options) {
    ...
    return this.each(function () {
      ...
      function buildNav() {
        ...
        if (settings.nav === 'after') {self.append(_nav);} else
        {self.prepend(_nav);}
      ...
      function jump(to) {
        ...
        if (to === 'prev') {
          ...
          if (settings.loop) {_currentIndex = _items.length - 1;}
        } else if (to === 'next') {
          if (_currentIndex < _items.length - 1) {_currentIndex++;} else
          if (settings.loop) {_currentIndex = 0;}
